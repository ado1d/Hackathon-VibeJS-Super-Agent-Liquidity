...
async def test_scenario_a_creates_provider_shortage(session) -> None:
    ...
    count = (await session.execute(select(func.count(Alert.id)).where(
        Alert.scenario_run_id == run.id, Alert.alert_type == "provider_liquidity"))).scalar_one()
    assert count >= 1


async def test_scenario_b_creates_cash_and_unusual_alerts(session) -> None:
    run = await load_scenario(session, "B")
    types = set((await session.execute(select(Alert.alert_type).where(Alert.scenario_run_id == run.id))).scalars())
...
async def test_scenario_c_suppresses_precise_forecast(session) -> None:
    ...
    rows = (await session.execute(select(Forecast).where(Forecast.scenario_run_id == run.id,
                                                          Forecast.resource_type == "provider_e_money"))).scalars().all()
    assert any(not row.reliable and row.shortage_minutes is None and row.severity == Severity.DATA_ISSUE for row in rows)
