...
async def main() -> None:
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        login = await client.post("/api/v1/auth/login", json={"username": "operations", "password": "demo-pass"})
        ...
        assert (await client.post(f"/api/v1/alerts/{alert_id}/claim", headers=headers)).status_code == 200
        assert (await client.post(f"/api/v1/alerts/{alert_id}/acknowledge", headers=headers)).status_code == 200
        assert (await client.post(f"/api/v1/alerts/{alert_id}/notes", headers=headers,
                                  json={"content": "Agent contacted using the approved support process."})).status_code == 200
        assert (await client.post(f"/api/v1/alerts/{alert_id}/escalate", headers=headers,
                                  json={"assigned_role": "risk", "note": "Review requested."})).status_code == 200
        switched = await client.post("/api/v1/auth/switch-role", headers=headers, json={"role": "risk"})
        risk_headers = {"Authorization": f"Bearer {switched.json()['access_token']}"}
        assert (await client.post(f"/api/v1/alerts/{alert_id}/in-progress", headers=risk_headers)).status_code == 200
        resolved = await client.post(f"/api/v1/alerts/{alert_id}/resolve", headers=risk_headers,
                                     json={"resolution_code": "reviewed_no_further_action",
        ...
        admin_login = await client.post("/api/v1/auth/login",
                                        json={"username": "admin", "password": "demo-pass"})
        admin_headers = {"Authorization": f"Bearer {admin_login.json()['access_token']}"}
        imported = await client.post("/api/v1/admin/transactions/import", headers=admin_headers,
                                     json=[{
                                         "external_event_id": "API-SMOKE-1", "agent_code": "AG-1001",
                                         "provider_code": "PROVIDER_A", "transaction_type": "cash_in",
                                         "amount": "500.00", "status": "success",
                                         "synthetic_customer_id": "SYN-API-1",
                                         "occurred_at": "2026-07-11T10:31:00+06:00",
                                     }, {
                                         "external_event_id": "API-SMOKE-BAD", "agent_code": "AG-1001",
                                         "provider_code": "PROVIDER_A", "transaction_type": "cash_in",
        ...
        assert imported.json() == {"accepted": 1, "quarantined": 1,
