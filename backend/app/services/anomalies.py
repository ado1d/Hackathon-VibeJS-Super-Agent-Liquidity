...
def successful(items: Iterable[Any]) -> list[Any]:
    return [
        item
        for item in items
        if str(_v(item, "status")) in {"success", TransactionStatus.SUCCESS.value}
    ]
...
def repeated_near_identical(items: Iterable[Any], config: dict[str, Any]) -> Detection | None:
    ...
    window = [
        t for t in txs if _v(t, "occurred_at") >= end - timedelta(minutes=cfg["window_minutes"])
    ]
    ...
    if max(amounts) - min(amounts) > tolerance * 2 or total < Decimal(
        str(cfg["minimum_total_bdt"])
    ):
        return None
    return Detection(
        ...
        "Repeated near-identical transactions require contextual review.",
        {
            ...
            "amount_min_bdt": float(min(amounts)),
            "amount_max_bdt": float(max(amounts)),
            "identifier_count": len(ids),
            "total_bdt": float(total),
...
def velocity_spike(
    items: Iterable[Any], config: dict[str, Any], baseline_counts: list[int] | None = None
) -> Detection | None:
    ...
    return Detection(
        "velocity_spike",
        Severity.MEDIUM,
        "Transaction velocity is above the synthetic baseline.",
        {
...
def identifier_concentration(items: Iterable[Any], config: dict[str, Any]) -> Detection | None:
    ...
    return Detection(
        ...
        {
            "top_identifiers": [key for key, _ in top],
            "value_percent": round(percent, 2),
            "threshold_percent": cfg["value_percent"],
            "total_bdt": overall,
        },
    )
...
def failure_rate_spike(items: Iterable[Any], config: dict[str, Any]) -> Detection | None:
    txs = list(items)
    failures = [
        t for t in txs if str(_v(t, "status")) in {"failed", TransactionStatus.FAILED.value}
    ]
    cfg = config["failure"]
    ...
    return Detection(
        ...
        "The recent failure rate is above the configured review threshold.",
        {
            ...
            "threshold_percent": cfg["rate_percent"],
        },
    )
...
def ledger_balance_conflict(
    opening: Decimal, reported: Decimal, items: Iterable[Any], config: dict[str, Any]
) -> Detection | None:
    expected = opening
    ...
    if difference <= Decimal(str(config["ledger_conflict"]["tolerance_bdt"])):
        return None
    return Detection(
        "ledger_balance_conflict",
        Severity.DATA_ISSUE,
        "Reported provider balance conflicts with successful ledger movement.",
        {
            "opening_balance_bdt": float(opening),
            "expected_balance_bdt": float(expected),
            "reported_balance_bdt": float(reported),
            "difference_bdt": float(difference),
        },
    )


def isolation_forest_detection(
    items: Iterable[Any], config: dict[str, Any], enabled: bool = True
) -> Detection | None:
    txs = list(items)
    ...
    features = np.array(
        [
            [
                float(_v(t, "amount")),
                _v(t, "occurred_at").hour,
                1.0 if str(_v(t, "status")) == "failed" else 0.0,
            ]
            for t in txs
        ]
    )
    model = IsolationForest(
        contamination=config["isolation_forest"]["contamination"], random_state=20260711
    )
    ...
    return Detection(
        ...
        "A secondary statistical scorer found a transaction requiring review.",
        {
            ...
            "observed_features": {
                "amount_bdt": features[worst][0],
                "hour": features[worst][1],
                "failed": bool(features[worst][2]),
            },
            "baseline_percentiles": {
                "p25": baseline[0].tolist(),
                "p50": baseline[1].tolist(),
...
def run_detectors(
    items: Iterable[Any], config: dict[str, Any], enable_iforest: bool = True
) -> list[Detection]:
    txs = list(items)
    results = [
        repeated_near_identical(txs, config),
        velocity_spike(txs, config),
        identifier_concentration(txs, config),
        failure_rate_spike(txs, config),
        outside_operating_hours(txs, config),
        isolation_forest_detection(txs, config, enable_iforest),
    ]
