...
def validate_transition(current: AlertStatus, target: AlertStatus) -> None:
    if target not in TRANSITIONS[current]:
        raise AppError(
            ...
            f"An alert in '{current.value}' cannot transition to '{target.value}'.",
            409,
...
async def transition(
    ...
    if target == AlertStatus.RESOLVED and (
        resolution_code is None or not note or len(note.strip()) < 3
    ):
    ...
    add_alert_event(
        session, alert.id, "status_changed", actor.id, previous.value, target.value, details
    )
    add_audit(
        session,
...
async def add_note(
    session: AsyncSession, alert: Alert, actor: User, content: str, note_type: str = "case_note"
) -> CaseNote:
    note = CaseNote(
        alert_id=alert.id, author_user_id=actor.id, content=content, note_type=note_type
    )
    session.add(note)
    add_alert_event(
        session,
...
async def upsert_alert(
    ...
    *,
    ...
    assigned_role: Role = Role.OPERATIONS,
    cooldown_minutes: int = 30,
) -> Alert:
    cutoff = utcnow() - timedelta(minutes=cooldown_minutes)
    provider_match = (
        Alert.provider_id.is_(None) if provider_id is None else Alert.provider_id == provider_id
    )
    query = (
        select(Alert)
        .where(
            and_(
                ...
                Alert.last_detected_at >= cutoff,
            )
        )
        .order_by(Alert.last_detected_at.desc())
    )
    ...
    if existing:
        ...
        add_alert_event(
            session, existing.id, "detection_refreshed", details={"severity": severity.value}
        )
        return existing
    alert = Alert(
        agent_id=agent_id,
