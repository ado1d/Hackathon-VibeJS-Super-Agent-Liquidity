...
def _response(user: User) -> TokenResponse:
    ...
    view = UserView.model_validate(user).model_copy(update={"permissions": permissions_for(user.role)})
...
async def login(payload: LoginRequest, request: Request, session: AsyncSession = Depends(get_session)) -> TokenResponse:
    ...
    add_audit(session, "auth.login", user.id, "user", str(user.id), request.state.request_id,
...
async def switch_role(payload: SwitchRoleRequest, request: Request, actor: User = Depends(current_user),
                      session: AsyncSession = Depends(get_session)) -> TokenResponse:
    if get_settings().app_env != "demo":
        raise AppError("AUTH_ROLE_SWITCH_DISABLED", "Role switching is only available in demo mode.", 403)
    target = (await session.execute(select(User).where(User.role == payload.role, User.is_active.is_(True)))).scalars().first()
    if target is None:
        raise AppError("AUTH_ROLE_UNAVAILABLE", "No active demo account exists for that role.", 404)
    add_audit(session, "auth.role_switched", actor.id, "user", str(target.id), request.state.request_id,
