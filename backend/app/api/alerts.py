...
async def scoped_alert(session: AsyncSession, alert_id: uuid.UUID, user: User) -> Alert:
    ...
    allowed = user.role == Role.ADMIN or (user.role == Role.AGENT and agent and agent.user_id == user.id) or (
        user.role == Role.OPERATIONS and agent and agent.assigned_operations_user_id == user.id) or (
        user.role == Role.RISK and (alert.assigned_role == Role.RISK or alert.status == AlertStatus.ESCALATED))
    if not allowed:
...
async def list_alerts(page: int = Query(1, ge=1), page_size: int = Query(25, ge=1, le=100),
                      provider_id: uuid.UUID | None = None, agent_id: uuid.UUID | None = None,
    ...
    if user.role == Role.AGENT:
        ...
        query = query.where((Alert.assigned_role == Role.RISK) | (Alert.status == AlertStatus.ESCALATED))
    elif user.role == Role.MANAGEMENT:
        raise AppError("AUTH_FORBIDDEN", "Management access is limited to aggregate summaries.", 403)
    if provider_id:
    ...
    items = (await session.execute(query.order_by(Alert.severity, Alert.created_at.desc()).offset(
        (page - 1) * page_size).limit(page_size))).scalars().all()
    return {"items": items, "page": page, "page_size": page_size, "total": total}


@router.get("/{alert_id}", response_model=None)
async def detail(alert_id: uuid.UUID, user: User = Depends(current_user),
                 session: AsyncSession = Depends(get_session)) -> dict:
    alert = await scoped_alert(session, alert_id, user)
    notes = (await session.execute(select(CaseNote).where(CaseNote.alert_id == alert.id)
                                   .order_by(CaseNote.created_at))).scalars().all()
    events = (await session.execute(select(AlertEvent).where(AlertEvent.alert_id == alert.id)
                                    .order_by(AlertEvent.occurred_at))).scalars().all()
    return {"alert": alert, "notes": notes, "events": events,
            "explanation_complete": all([alert.reason, alert.evidence, alert.uncertainty_statement,
                                          alert.recommended_next_step])}


@router.get("/{alert_id}/events", response_model=None)
async def events(alert_id: uuid.UUID, user: User = Depends(current_user),
                 session: AsyncSession = Depends(get_session)) -> list[AlertEvent]:
    await scoped_alert(session, alert_id, user)
    return list((await session.execute(select(AlertEvent).where(AlertEvent.alert_id == alert_id)
                                       .order_by(AlertEvent.occurred_at))).scalars().all())


@router.get("/{alert_id}/events.csv")
async def events_csv(alert_id: uuid.UUID, user: User = Depends(current_user),
                     session: AsyncSession = Depends(get_session)) -> StreamingResponse:
    rows = await events(alert_id, user, session)
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(["occurred_at", "event_type", "actor_user_id", "from_status", "to_status", "details"])
    for row in rows:
        writer.writerow([row.occurred_at.isoformat(), row.event_type, row.actor_user_id,
                         row.from_status, row.to_status, row.details])
...
async def claim_alert(alert_id: uuid.UUID, user: User = Depends(current_user),
                      session: AsyncSession = Depends(get_session)) -> Alert:
...
async def acknowledge(alert_id: uuid.UUID, user: User = Depends(current_user),
                      session: AsyncSession = Depends(get_session)) -> Alert:
    return await transition(session, await scoped_alert(session, alert_id, user), AlertStatus.ACKNOWLEDGED, user)


@router.post("/{alert_id}/in-progress", response_model=None)
async def in_progress(alert_id: uuid.UUID, user: User = Depends(current_user),
                      session: AsyncSession = Depends(get_session)) -> Alert:
    if user.role not in {Role.OPERATIONS, Role.RISK, Role.ADMIN}:
        raise AppError("AUTH_FORBIDDEN", "Your role cannot manage operational status.", 403)
    return await transition(session, await scoped_alert(session, alert_id, user), AlertStatus.IN_PROGRESS, user)


@router.post("/{alert_id}/escalate", response_model=None)
async def escalate(alert_id: uuid.UUID, payload: EscalateRequest, user: User = Depends(current_user),
                   session: AsyncSession = Depends(get_session)) -> Alert:
    if user.role not in {Role.OPERATIONS, Role.RISK, Role.ADMIN}:
...
async def resolve(alert_id: uuid.UUID, payload: ResolveRequest, user: User = Depends(current_user),
                  session: AsyncSession = Depends(get_session)) -> Alert:
    if user.role not in {Role.OPERATIONS, Role.RISK, Role.ADMIN}:
        raise AppError("AUTH_FORBIDDEN", "Your role cannot resolve operational cases.", 403)
    return await transition(session, await scoped_alert(session, alert_id, user), AlertStatus.RESOLVED, user,
                            payload.resolution_code, payload.note)


@router.post("/{alert_id}/reopen", response_model=None)
async def reopen(alert_id: uuid.UUID, user: User = Depends(current_user),
                 session: AsyncSession = Depends(get_session)) -> Alert:
    if user.role not in {Role.OPERATIONS, Role.RISK, Role.ADMIN}:
        raise AppError("AUTH_FORBIDDEN", "Your role cannot reopen cases.", 403)
    return await transition(session, await scoped_alert(session, alert_id, user), AlertStatus.REOPENED, user)


@router.post("/{alert_id}/notes", response_model=None)
async def create_note(alert_id: uuid.UUID, payload: NoteRequest, user: User = Depends(current_user),
                      session: AsyncSession = Depends(get_session)) -> CaseNote:
    alert = await scoped_alert(session, alert_id, user)
